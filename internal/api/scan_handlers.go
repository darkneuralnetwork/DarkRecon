...
import (
	...
	"github.com/yourname/dark-recon/internal/auth"
...
type Handlers struct {
	...
	// auth enforces session-based authentication on the web UI / REST API.
	// May be nil when auth is disabled, but the middleware is a no-op in that
	// case so handlers can assume a valid (or absent) session either way.
	auth *auth.Auth
	...
	// fwd is the lazily-created Repeater forwarder. Shared across targets; the
	// per-request scope checker is bound at send time via SetScope.
	fwd *repeater.Forwarder
...
// New creates a new API handlers instance. auth may be nil only when
// authentication is disabled; callers should still pass a non-nil instance so
// the login/logout endpoints remain wired (they no-op when disabled).
func New(cfg *config.Config, configPath string, sm *scanmgr.Manager, a *auth.Auth) *Handlers {
	return &Handlers{
		cfg:        cfg,
