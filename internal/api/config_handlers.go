...
// BrowseFiles — GET /api/fs/files?path=/usr/share/wordlists
//
// Lists the subdirectories AND files in a path, for the Repeater Intruder's
// wordlist picker. Unlike BrowseDirs (which only returns directories for the
// Settings page), this returns files too — with their size — so the user can
// pick a wordlist file. Hidden entries (starting with '.') are skipped.
// Symlinks are followed (os.Stat resolves the target) so Kali's
// /usr/share/wordlists/* symlink farm is navigable.
func (h *Handlers) BrowseFiles(w http.ResponseWriter, r *http.Request) {
	path := r.URL.Query().Get("path")
	if path == "" {
		path = "/"
	}
	...
	if info, err := os.Stat(path); err == nil && info.IsDir() {
		if entries, err := os.ReadDir(path); err == nil {
			for _, e := range entries {
				...
				if fi.IsDir() {
					dirs = append(dirs, entry{Name: name, Path: full})
				} else {
					files = append(files, entry{Name: name, Path: full, Size: fi.Size()})
				}
			}
		}
	}
