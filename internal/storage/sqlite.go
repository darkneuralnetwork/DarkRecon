...
func (db *DB) migrate() error {
	...
	-- Repeater: saved raw HTTP requests + their last response. One row per
	-- saved request; re-sending updates the response_* columns in place.
	-- Scoped to a scan so requests travel with the target they belong to.
	...
	CREATE INDEX IF NOT EXISTS idx_repeater_scan ON repeater_requests(scan_id);
	CREATE INDEX IF NOT EXISTS idx_repeater_sub ON repeater_requests(subdomain);
	`
...
// GetParamsForURL returns the param names discovered (by arjun/katana) for a
// specific URL within a scan, as a name→type map. Used by the Repeater to
// pre-populate a raw request with every known parameter so the user can test
// them without re-discovering them. Values are empty strings (placeholders).
func (db *DB) GetParamsForURL(scanID int64, url string) map[string]string {
	rows, err := db.conn.Query(
		`SELECT param_name, param_type FROM p1_params WHERE scan_id = ? AND url = ?`,
		scanID, url,
	)
	if err != nil {
		return nil
	...
	for rows.Next() {
		...
		if err := rows.Scan(&name, &ptype); err != nil {
			continue
		}
		if _, exists := out[name]; !exists {
			out[name] = "" // placeholder value; user fills in
		}
	}
	return out
}
...
// CreateRepeaterRequest inserts a new saved Repeater request and returns its ID.
func (db *DB) CreateRepeaterRequest(req *RepeaterRequest) (int64, error) {
	res, err := db.conn.Exec(
		`INSERT INTO repeater_requests (scan_id, target, subdomain, url, method, raw_request, notes)
		 VALUES (?, ?, ?, ?, ?, ?, ?)`,
		req.ScanID, req.Target, req.Subdomain, req.URL, req.Method, req.RawRequest, req.Notes,
	)
	if err != nil {
		return 0, err
	}
...
// UpdateRepeaterRequest saves an edited raw request, method, url, subdomain
// and notes for an existing Repeater entry. Response columns are untouched.
func (db *DB) UpdateRepeaterRequest(req *RepeaterRequest) error {
	_, err := db.conn.Exec(
		`UPDATE repeater_requests
		   SET raw_request = ?, method = ?, url = ?, subdomain = ?, notes = ?, updated_at = CURRENT_TIMESTAMP
		 WHERE id = ? AND scan_id = ?`,
		req.RawRequest, req.Method, req.URL, req.Subdomain, req.Notes, req.ID, req.ScanID,
	)
	return err
}

// UpdateRepeaterResponse stores the response captured when a request was sent.
// Any of the response fields may be NULL (e.g. an error before any response).
func (db *DB) UpdateRepeaterResponse(scanID, id int64, status *int, headers, body, errMsg *string, durationMs *int64) error {
	_, err := db.conn.Exec(
		`UPDATE repeater_requests
		   SET response_status = ?, response_headers = ?, response_body = ?, response_error = ?, duration_ms = ?, updated_at = CURRENT_TIMESTAMP
		 WHERE id = ? AND scan_id = ?`,
		status, headers, body, errMsg, durationMs, id, scanID,
	)
	return err
}

// GetRepeaterRequest returns a single Repeater entry by ID (scoped to scanID).
func (db *DB) GetRepeaterRequest(scanID, id int64) (*RepeaterRequest, error) {
	row := db.conn.QueryRow(
		`SELECT id, scan_id, target, subdomain, url, method, raw_request,
		        response_status, response_headers, response_body, response_error,
		        duration_ms, notes, created_at, updated_at
		 FROM repeater_requests WHERE id = ? AND scan_id = ?`, id, scanID,
	)
	return scanRepeaterRequest(row)
}

// ListRepeaterRequests returns every saved Repeater request for a scan, most
// recent first. Used to populate the history pane.
func (db *DB) ListRepeaterRequests(scanID int64) ([]RepeaterRequest, error) {
	rows, err := db.conn.Query(
		`SELECT id, scan_id, target, subdomain, url, method, raw_request,
		        response_status, response_headers, response_body, response_error,
		        duration_ms, notes, created_at, updated_at
		 FROM repeater_requests WHERE scan_id = ?
		 ORDER BY updated_at DESC`, scanID,
	)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var out []RepeaterRequest
	for rows.Next() {
		r, err := scanRepeaterRequest(rows)
		if err != nil {
			continue
		}
		out = append(out, *r)
...
// DeleteRepeaterRequest removes a saved Repeater entry (scoped to scanID).
func (db *DB) DeleteRepeaterRequest(scanID, id int64) error {
	_, err := db.conn.Exec(`DELETE FROM repeater_requests WHERE id = ? AND scan_id = ?`, id, scanID)
	return err
}

// IsHostInScope reports whether host appears in the scan's live_hosts or
// crawled_urls tables, either as a subdomain value or as the host portion of a
// URL. This is the SSRF / scope guard for the Repeater forwarder: only hosts
// already discovered during recon may be sent to. The host may include a port;
// the port is stripped before comparison. Comparison is case-insensitive.
func (db *DB) IsHostInScope(scanID int64, host string) (bool, error) {
	...
	err := db.conn.QueryRow(`
		SELECT EXISTS(
			SELECT 1 FROM (
				SELECT subdomain AS h, url AS u FROM live_hosts   WHERE scan_id = ?
				UNION ALL
				SELECT subdomain AS h, url AS u FROM crawled_urls WHERE scan_id = ?
			)
			WHERE lower(h) = ?
			   OR lower(u) = 'http://'  || ?
			   OR lower(u) = 'https://' || ?
			   OR lower(u) LIKE 'http://'  || ? || '/%'
			   OR lower(u) LIKE 'http://'  || ? || ':%'
			   OR lower(u) LIKE 'https://' || ? || '/%'
			   OR lower(u) LIKE 'https://' || ? || ':%'
		)`,
		scanID, scanID, host, host, host, host, host, host, host,
	).Scan(&ok)
	return ok, err
}

// scanner is the common interface satisfied by both *sql.Row and *sql.Rows,
// letting scanRepeaterRequest share one implementation between Get and List.
type scanner interface {
	Scan(dest ...any) error
}

func scanRepeaterRequest(s scanner) (*RepeaterRequest, error) {
	r := &RepeaterRequest{}
	var subdomain, url, respHeaders, respBody, respErr sql.NullString
	var respStatus sql.NullInt64
	var durationMs sql.NullInt64
	var created, updated sql.NullString
	if err := s.Scan(
		&r.ID, &r.ScanID, &r.Target, &subdomain, &url, &r.Method, &r.RawRequest,
		&respStatus, &respHeaders, &respBody, &respErr, &durationMs,
		&r.Notes, &created, &updated,
	); err != nil {
		return nil, err
	}
	if subdomain.Valid {
