...
// ── Repeater (Phase-2 manual testing) ────────────────────────────

// RepeaterRequest stores a raw HTTP request saved to the Repeater, along
// with the most recent response captured when it was sent. One row per saved
// request; re-sending updates the response columns in place. Scoped to a scan
// so requests only appear alongside the target they were created from.
