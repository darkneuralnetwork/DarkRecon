...
func (e *Engine) runVulnPhase(ctx context.Context, p1 *phasemod.Runner, liveResult *discovery.LiveResult) error {
	...
	// ── Group AB & Group C (ALL CONCURRENT) ──
		...
		if groupCActive {
			...
			if runSecrets {
				...
				go func() {
					...
					p1.RunSecretScan(vulnCtx)
				}()
			}
		}

		// Wait for all goroutines from Group AB and Group C to finish
		wg.Wait()
