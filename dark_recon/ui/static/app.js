...
function injectTopbar() {
  ...
  <header class="topbar">
    ...
    <nav class="topbar-nav">
      ...
      <button class="nav-link" id="repeater-nav-btn" type="button" title="Open the Repeater manual request editor">
        <span class="nav-icon">${ICON.bolt}</span><span>Repeater</span>
      </button>
    ...
    <div class="topbar-actions">
      ...
      <div class="dropdown" id="user-dropdown">
        <button class="nav-link" id="user-dropdown-btn" type="button" title="Account">
          <span class="nav-icon">${ICON.shield}</span><span id="current-user" class="user-label">…</span>
          <span class="nav-icon" style="opacity:0.5">${ICON.chevron}</span>
        </button>
        <div class="dropdown-panel" id="user-panel">
          <h4>Signed in as</h4>
          <p id="user-panel-name" class="text-muted">…</p>
          <a href="/account" class="dropdown-link" style="display:flex;align-items:center;gap:8px;padding:9px 12px;border-radius:var(--radius-sm);color:var(--text-secondary);font-size:0.88em;transition:var(--transition-fast);text-decoration:none;">
            <span class="nav-icon">${ICON.settings}</span><span>Account &amp; Password</span>
          </a>
          <button class="btn btn-secondary" id="logout-btn" type="button" style="width:100%;margin-top:6px;justify-content:center;">
            ${ICON.close}<span>Sign Out</span>
          </button>
        </div>
      </div>
    </div>
...
async function initUserMenu() {
  ...
  // Confirm the session and populate the username. If the session is gone
  // (e.g. expired), redirect to login.
  try {
  ...
  if (btn && dropdown) {
    btn.addEventListener('click', (e) => {
      e.stopPropagation();
      // Toggle `open` on the PARENT .dropdown container — the CSS rule
      // `.dropdown.open .dropdown-panel` shows the panel based on this
      // class, not on the panel itself.
      dropdown.classList.toggle('open');
      const tlist = document.getElementById('targets-dropdown');
      if (tlist) tlist.classList.remove('open');
    });
...
// ════════════════════════════════════════════════════════════════
//  GLOBAL 401 HANDLING
//  Wraps window.fetch so any API call that returns 401 redirects to the
//  login page (preserving the current URL as ?next=). Avoids the raw
//  JSON 401 body leaking to users when a session expires mid-session.
// ════════════════════════════════════════════════════════════════
(function wrapFetch() {
...
function initTargetsDropdown() {
  ...
  btn.addEventListener('click', (e) => {
    ...
    if (udropdown) udropdown.classList.remove('open');
  });
  document.addEventListener('click', (e) => {
    if (!dropdown.contains(e.target)) dropdown.classList.remove('open');
  });
...
function initRepeaterNav() {
  const btn = document.getElementById('repeater-nav-btn');
  if (!btn) return;
  btn.addEventListener('click', () => {
    const m = window.location.pathname.match(/^\/target\/([^/]+)/);
    if (m) {
      const target = decodeURIComponent(m[1]);
      window.location.href = `/target/${encodeURIComponent(target)}/repeater`;
      return;
    }
    
    // Not on a target page — open a modal to select a target
    let modal = document.getElementById('repeater-modal');
    if (modal) {
      modal.classList.add('visible');
    } else {
      modal = document.createElement('div');
      modal.id = 'repeater-modal';
      modal.className = 'modal-overlay visible';
      modal.innerHTML = `
        <div class="modal" style="max-width: 400px">
          <div class="modal-header">
            <h3>Open Repeater</h3>
            <button class="modal-close" onclick="document.getElementById('repeater-modal').classList.remove('visible')">&times;</button>
          </div>
          <div class="modal-body">
            <p class="text-muted" style="margin-bottom:16px;font-size:0.9em;">Select a target to open its manual request editor:</p>
            <div id="repeater-modal-list" style="display:flex;flex-direction:column;gap:8px;max-height:300px;overflow-y:auto;padding-right:4px;">
               <span class="spinner" style="margin:auto"></span>
            </div>
          </div>
        </div>
      `;
    ...
    fetch('/api/targets').then(r => r.json()).then(data => {
       const list = document.getElementById('repeater-modal-list');
       const targets = data.targets || [];
       if (targets.length === 0) {
          list.innerHTML = '<p class="text-muted">No targets available. Launch a scan first.</p>';
          return;
       }
       list.innerHTML = targets.map(t => 
         `<a href="/target/${encodeURIComponent(t.name)}/repeater" class="btn btn-secondary" style="justify-content:flex-start;font-weight:600;padding:12px;">${escapeHtml(t.name)} <span style="margin-left:auto;opacity:0.5">→</span></a>`
       ).join('');
    }).catch(() => {
       document.getElementById('repeater-modal-list').innerHTML = '<p class="text-muted" style="color:var(--critical)">Failed to load targets.</p>';
    });
  });
}
