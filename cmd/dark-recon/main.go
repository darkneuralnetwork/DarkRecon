...
func main() {
	...
	// Build the authenticator from config. Enabled by default with
	// admin/admin; the password is bcrypt-hashed at startup so the plaintext
	// is never retained in memory. See internal/auth.
	authInst, err := auth.New(cfg)
	if err != nil {
		logger.Err("Failed to initialize auth: %v", err)
		os.Exit(1)
	}

	// Create API handlers
	handlers := api.New(cfg, *configPath, scanMgr, authInst)

	// Register routes
	mux := http.NewServeMux()
	handlers.RegisterRoutes(mux, *templateDir, *staticDir)

	// Wrap the entire mux with the auth middleware. Public paths
	// (/login, /logout, /static/, /favicon.ico) bypass auth inside the
	// middleware; everything else requires a valid session cookie.
	var handler http.Handler = mux
	if authInst.Enabled() {
		handler = authInst.Middleware(mux)
		logger.Success("Auth middleware active — login required at http://localhost:%d", *port)
	}
